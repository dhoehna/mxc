 ...
 */
function isSystemCriticalPath(dirPath) {
 ...
 */
function hasAllApplicationPackagesAccess(dirPath) {
    if (os.platform() !== 'win32') {
 ...
 */
function getPowerShellPolicy(pathDirs, env) {
    if (os.platform() !== 'win32') {
    ...
    const readonlyPaths = [systemRoot];
    const readwritePaths = [];
    const userProfile = env['USERPROFILE'];
    if (userProfile) {
        const psReadLineDir = path.join(userProfile, 'AppData', 'Roaming', 'Microsoft', 'Windows', 'PowerShell', 'PSReadLine');
 ...
 */
export function getAvailableToolsPolicy(env, options) {
 ...
 */
export function getUserProfilePolicy() {
    const readonlyPaths = [];
    if (os.platform() === 'win32') {
 ...
 */
export function getTemporaryFilesPolicy(env) {
    const environment = env ?? process.env;
