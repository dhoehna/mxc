 ...
 */
function defaultWindowsBuildQuery() {
...
let windowsBuildQuery = defaultWindowsBuildQuery;
/** @internal Test-only: override the Windows build lookup. */
export function _setWindowsBuildQuery(fn) {
 ...
 */
function isIsoSessionSupported() {
...
let windowsSandboxAvailableCache;
/**
...
function isWindowsSandboxAvailable() {
    ...
    try {
        const output = execSync('dism /online /get-featureinfo /featurename:Containers-DisposableClientVM', { encoding: 'utf-8', stdio: 'pipe', timeout: 10000 });
...
export function _resetPlatformSupportCache() {
    cachedSupport = null;
}
let probeRunner = defaultProbeRunner;
/** @internal Test-only: override the probe runner. */
export function _setProbeRunner(runner) {
...
function isUiCapabilitySupport(value) {
 ...
 * pre-Phase-5 SDKs.
 */
function populateIsolationFromProbe(support) {
    try {
        ...
        if (probe && typeof probe === 'object') {
            ...
            if (Array.isArray(probe.warnings) && probe.warnings.length > 0) {
                const warnings = probe.warnings.filter((w) => typeof w === 'string');
...
function computeSupport() {
    ...
    const support = { isSupported: false, reason: '', availableMethods: [] };
 ...
 */
function getLinuxRustTargetTriple() {
    const arch = os.arch();
    switch (arch) {
        case 'arm64':
 ...
 */
function getDarwinRustTargetTriple() {
    const arch = os.arch();
 ...
 */
export function findWxcExecutable() {
 ...
 * @returns true if the executable exists and is a file, false otherwise
 */
function verifyWxcExecutable(wxcPath) {
 ...
 */
export function findLxcExecutable() {
    // Allow override for bundled deployments (debugging/testing)
    if (process.env.MXC_BIN_DIR) {
 ...
 */
export function findSeatbeltExecutable() {
    // Allow override for bundled deployments (debugging/testing)
    if (process.env.MXC_BIN_DIR) {
