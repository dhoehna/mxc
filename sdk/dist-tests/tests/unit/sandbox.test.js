...
import { buildSandboxPayload, createConfigFromPolicy, spawnSandboxFromConfig } from '../../src/sandbox.js';
import { resolveExecutableAndArgs } from '../../src/helper.js';
import { platformSkip } from './test-helpers.js';
describe('buildSandboxPayload', () => {
    const defaultPolicy = { version: '0.4.0-alpha' };
    describe('Windows', () => {
        ...
        it('should reject a newer minor version within same major', () => {
            mockWindows();
            try {
                assert.throws(() => buildSandboxPayload('echo hi', { version: '0.99.0' }), { message: /newer than supported/ });
            }
            finally {
                restore();
            }
        ...
        it('should reject a different major version', () => {
            mockWindows();
            try {
                assert.throws(() => buildSandboxPayload('echo hi', { version: '1.0.0' }), { message: /newer than supported/ });
            }
            finally {
                restore();
            }
        ...
        it('should reject a version older than minimum', () => {
            mockWindows();
            try {
                assert.throws(() => buildSandboxPayload('echo hi', { version: '0.3.0-alpha' }), { message: /older than supported/ });
            }
            finally {
                restore();
            }
        ...
        it('should reject an invalid semver string', () => {
            mockWindows();
            try {
                assert.throws(() => buildSandboxPayload('echo hi', { version: 'not-a-version' }), { message: /Invalid policy version/ });
            }
            finally {
                restore();
            }
        ...
        it('should reject an empty version string', () => {
            mockWindows();
            try {
                assert.throws(() => buildSandboxPayload('echo hi', { version: '' }), { message: /version is required/ });
            }
            finally {
                restore();
            }
    ...
    describe('Linux', () => {
        ...
        it('should reject proxy configuration on Linux for non-bubblewrap containments (e.g. lxc)', () => {
            ...
            try {
                ...
                assert.throws(() => buildSandboxPayload('echo hi', policy, undefined, undefined, 'lxc'), { message: /not supported on Linux containment='lxc'/ });
            }
            finally {
                restore();
            }
    ...
    describe('Containment override', () => {
        ...
        it('should reject network policies for microvm', () => {
            ...
            try {
                ...
                assert.throws(() => buildSandboxPayload('print(42)', policy, undefined, undefined, 'microvm'), { message: /does not support network policy/ });
            }
            finally {
                restore();
            }
        ...
        it('should reject microvm on non-Windows platforms', () => {
            ...
            Object.defineProperty(process, 'platform', { value: 'linux' });
            try {
                assert.throws(() => buildSandboxPayload('print(42)', defaultPolicy, undefined, undefined, 'microvm'), { message: /only supported on Windows/ });
            }
            finally {
                if (orig)
...
describe('createConfigFromPolicy', () => {
        ...
        it('should reject proxy on Linux for explicit lxc containment', () => {
            ...
            try {
                assert.throws(() => createConfigFromPolicy({
                    version: '0.4.0-alpha',
                    network: { proxy: { builtinTestServer: true } },
                }, 'lxc'), { message: /not supported on Linux containment='lxc'/ });
            }
            finally {
                restore();
            }
    ...
    describe('WSLC', () => {
        ...
        it('should throw from spawnSandbox when experimental backend is used via config', () => {
            ...
            assert.throws(() => spawnSandboxFromConfig(config), { message: /experimental mode/ });
        });
...
describe('resolveExecutableAndArgs (containment validation)', { skip: platformSkip }, () => {
    ...
    function makeConfig(containment) {
        return {
            version: '0.5.0-alpha',
    ...
    it('should accept the abstract intent "process" without throwing', () => {
        ...
        assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('process'), { executablePath: fakeExe }));
    });
    it('should accept the abstract intent "microvm" with experimental flag (Windows only)', function () {
        if (process.platform !== 'win32') {
            this.skip('microvm is Windows-only');
            return;
        }
        assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('microvm'), {
    ...
    it('should accept the abstract intent "vm" without throwing', () => {
        ...
        assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('vm'), { executablePath: fakeExe }));
    });
    it('should still reject genuinely unknown containment values', () => {
        assert.throws(() => resolveExecutableAndArgs(makeConfig('bogus_backend'), { executablePath: fakeExe }), { message: /not available on this platform/ });
    });
    it('should still require experimental mode for experimental backends like wslc', () => {
        assert.throws(() => resolveExecutableAndArgs(makeConfig('wslc'), { executablePath: fakeExe }), { message: /experimental mode/ });
    });
    it('should NOT require experimental mode for explicit lxc containment', function () {
        if (process.platform !== 'linux') {
            this.skip('lxc is Linux-only');
            return;
        }
        assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('lxc'), { executablePath: fakeExe }));
    });
    ...
    describe('legacy containment aliases', () => {
        it('should accept "appcontainer" as an alias of processcontainer (Windows)', function () {
            if (process.platform !== 'win32') {
                this.skip('processcontainer is Windows-only');
                return;
            }
            assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('appcontainer'), { executablePath: fakeExe }));
        });
        it('should reject "appcontainer" on non-Windows hosts with the canonical error', function () {
            if (process.platform === 'win32') {
                this.skip('appcontainer is the native value on Windows');
                return;
            }
            assert.throws(() => resolveExecutableAndArgs(makeConfig('appcontainer'), { executablePath: fakeExe }), { message: /'appcontainer' is not available on this platform/ });
        });
        it('should accept "macos_sandbox" on macOS', function () {
            if (process.platform !== 'darwin') {
                this.skip('seatbelt is macOS-only');
                return;
            }
            assert.doesNotThrow(() => resolveExecutableAndArgs(makeConfig('macos_sandbox'), { executablePath: fakeExe }));
        });
        it('should forward the legacy wire value to the binary unchanged', () => {
            ...
            makeConfig('appcontainer'), { executablePath: fakeExe, skipPlatformCheck: true });
    ...
    describe('builtinTestServer testing-features gate', () => {
        it('forwards --allow-testing-features when the caller opts in via allowTestingFeatures', () => {
            ...
            assert.ok(args.includes('--allow-testing-features'), 'expected --allow-testing-features to be forwarded');
        });
        it('throws when builtinTestServer is used without allowTestingFeatures', () => {
            ...
            assert.throws(() => resolveExecutableAndArgs(config, {
                executablePath: fakeExe,
                skipPlatformCheck: true,
        ...
        it('does not forward --allow-testing-features for a non-test proxy', () => {
            ...
            assert.ok(!args.includes('--allow-testing-features'), 'did not expect --allow-testing-features for a url proxy');
        });
    });
});
