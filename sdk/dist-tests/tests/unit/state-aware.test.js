...
import { deprovisionSandbox, execInSandboxAsync, provisionSandbox, startSandbox, stopSandbox, } from '../../src/state-aware.js';
import { _resetSpawnImpl, _setSpawnImpl, buildStateAwareEnvelope, parseNonExecResponse, } from '../../src/state-aware-helper.js';
import { MxcError } from '../../src/errors.js';
import { IsolationSessionUserConfig } from '../../src/state-aware-types.js';
...
describe('buildStateAwareEnvelope', () => {
    ...
    it('produces stop and deprovision envelopes carrying only version + phase + sandboxId', () => {
        for (const phase of ['stop', 'deprovision']) {
            const env = buildStateAwareEnvelope({
                phase,
                backendKey: 'isolation_session',
                sandboxId: 'iso:abc',
...
describe('parseNonExecResponse', () => {
    it('unwraps result payload', () => {
        const result = parseNonExecResponse('{"result":{"sandboxId":"iso:abc"}}');
    ...
    it('throws an MxcError carrying each wire error code', () => {
        ...
        for (const code of codes) {
            ...
            assert.throws(() => parseNonExecResponse(stdout), (err) => err instanceof MxcError && err.code === code);
        }
    });
    it('passes details through when wire envelope carries them', () => {
        ...
        assert.throws(() => parseNonExecResponse(stdout), (err) => {
    ...
    it('throws a plain Error on unparseable stdout', () => {
        assert.throws(() => parseNonExecResponse('not json'), (err) => {
...
describe('provisionSandbox', { skip: platformSkip }, () => {
    ...
    it('builds a provision envelope and unwraps the SandboxId from the response', async () => {
        ...
        const result = await provisionSandbox('isolation_session', { filesystem: { readwritePaths: ['C:\\workspace'] } }, testOptions());
    ...
    it('throws an MxcError carrying backend_unavailable when the executor reports it', async () => {
        ...
        activeFake = fake;
        _setSpawnImpl(fake.spawn);
        await assert.rejects(() => provisionSandbox('isolation_session', undefined, testOptions()), (err) => err instanceof MxcError && err.code === 'backend_unavailable');
    });
    it('rejects when AbortSignal fires before close', async () => {
        ...
        activeFake = fake;
        _setSpawnImpl(fake.spawn);
        const promise = provisionSandbox('isolation_session', undefined, testOptions({ signal: ac.signal }));
        ac.abort();
...
describe('stopSandbox', { skip: platformSkip }, () => {
    ...
    it('rejects with malformed_id when sandboxId has no recognised prefix', async () => {
        await assert.rejects(() => stopSandbox('not-a-real-id'), (err) => err instanceof MxcError && err.code === 'malformed_id');
        await assert.rejects(() => stopSandbox('unknownprefix:abc'), (err) => err instanceof MxcError && err.code === 'malformed_id');
    });
});
...
describe('execInSandboxAsync', { skip: platformSkip }, () => {
    ...
    it('returns ExecResult on successful script run', async () => {
        ...
        _setSpawnImpl(fake.spawn);
        const id = 'iso:abc';
        const result = await execInSandboxAsync(id, { process: { commandLine: 'echo hello' } }, testOptions());
    ...
    it('returns ExecResult on script exit != 0 when stdout is plain script output (not an error envelope)', async () => {
        ...
        _setSpawnImpl(fake.spawn);
        const id = 'iso:abc';
        const result = await execInSandboxAsync(id, { process: { commandLine: 'fail' } }, testOptions());
    ...
    it('throws the typed MxcError on dispatch failure (exit != 0 and stdout is a complete error envelope)', async () => {
        ...
        _setSpawnImpl(fake.spawn);
        const id = 'iso:abc';
        await assert.rejects(() => execInSandboxAsync(id, { process: { commandLine: 'echo' } }, testOptions()), (err) => err instanceof MxcError && err.code === 'stale_id');
    });
});
