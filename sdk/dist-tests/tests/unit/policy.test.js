...
describe('getAvailableToolsPolicy - PowerShell discovery', () => {
    ...
    it('should add system root to readonlyPaths when pwsh.exe is on PATH', { skip: isLinux }, () => {
        ...
        assert.ok(result.readonlyPaths.some(p => /^[a-z]:\\$/i.test(p)), 'System root (e.g. C:\\) should be in readonlyPaths when pwsh.exe is on PATH');
    });
    it('should add PSReadLine dir to readwritePaths when pwsh.exe is on PATH', { skip: isLinux }, () => {
        ...
        const env = { PATH: pwshDir, USERPROFILE: 'C:\\Users\\TestUser' };
        const result = getAvailableToolsPolicy(env);
        const expected = path.join('C:\\Users\\TestUser', 'AppData', 'Roaming', 'Microsoft', 'Windows', 'PowerShell', 'PSReadLine');
        assert.ok(result.readwritePaths.some(p => p.toLowerCase() === expected.toLowerCase()), 'PSReadLine directory should be in readwritePaths');
    });
    it('should not add PowerShell paths when pwsh.exe is not on PATH', { skip: isLinux }, () => {
        ...
        const result = getAvailableToolsPolicy(env);
        assert.ok(!result.readonlyPaths.some(p => /^[a-z]:\\$/i.test(p)), 'System root should not be in readonlyPaths when pwsh.exe is not on PATH');
        assert.strictEqual(result.readwritePaths.length, 0, 'readwritePaths should be empty when pwsh.exe is not on PATH');
    });
    it('should return empty policy on non-Windows even when pwsh.exe is on PATH', { skip: isLinux }, () => {
        ...
        const env = { PATH: pwshDir, USERPROFILE: 'C:\\Users\\TestUser' };
        const result = getAvailableToolsPolicy(env);
        assert.ok(!result.readonlyPaths.some(p => /^[a-z]:\\$/i.test(p)), 'System root (e.g. C:\\) should not be in readonlyPaths on Linux');
        assert.strictEqual(result.readwritePaths.length, 0, 'readwritePaths should be empty on Linux');
    });
    it('should not add PSReadLine path when USERPROFILE is not set', { skip: isLinux }, () => {
        ...
        const result = getAvailableToolsPolicy(env);
        assert.ok(result.readonlyPaths.some(p => /^[a-z]:\\$/i.test(p)), 'System root should still be in readonlyPaths');
        assert.strictEqual(result.readwritePaths.length, 0, 'readwritePaths should be empty without USERPROFILE');
    });
});
