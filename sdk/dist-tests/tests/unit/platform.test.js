...
import { getPlatformSupport, _resetPlatformSupportCache, _setProbeRunner, _setWindowsBuildQuery, findWxcExecutable, } from '../../src/platform.js';
...
describe('getPlatformSupport probe integration', () => {
    ...
    it('handles probe JSON with only `tier`', { skip: !isWindows }, () => {
        ...
        assert.strictEqual(support.isolationWarnings, undefined, 'missing warnings array must leave isolationWarnings undefined');
    });
...
describe('findWxcExecutable failure modes', () => {
    ...
    it('returns a string or null and never throws under a nonexistent MXC_BIN_DIR', () => {
        ...
        process.env.MXC_BIN_DIR = path.join(os.tmpdir(), `mxc-sdk-unit-no-such-dir-${process.pid}`);
...
describe('isolation_session availability gate', () => {
    ...
    it('omits isolation_session when minor build is below 8553', { skip: !isWindows }, () => {
        ...
        assert.ok(!support.availableMethods.includes('isolation_session'), `expected isolation_session absent, got: ${support.availableMethods.join(',')}`);
    });
