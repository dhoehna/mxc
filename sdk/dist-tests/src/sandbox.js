...
function validatePolicyVersion(version) {
    ...
    if (!parsed) {
        throw new Error(`Invalid policy version '${version}': must be valid semver` +
    ...
    if (parsed.major < minimum.major ||
        ...
        throw new Error(`Policy version '${version}' is older than supported` +
    ...
    if (parsed.major > supported.major ||
        ...
        throw new Error(`Policy version '${version}' is newer than supported` +
 ...
 */
function buildWslcContainerConfig(config, policy, containerId) {
 ...
 */
function buildBubblewrapConfig(config) {
 ...
 */
function buildLinuxProcessConfig(config) {
    config.lxc = {
        distribution: 'alpine',
        release: '3.23',
    };
 ...
 */
function buildDarwinProcessConfig(config) {
 ...
 */
function buildProcessBaseContainerConfig(config, policy) {
    const capabilities = [];
 ...
 */
function buildMicroVmConfig(config, policy) {
    if (os.platform() !== 'win32') {
        throw new Error('The microvm backend is only supported on Windows (requires WHP/Hyper-V).');
    }
    if (policy.network) {
        throw new Error('The microvm backend does not support network policy enforcement. ' +
 ...
 * ```
 */
export function createConfigFromPolicy(policy, containment = "process", containerName) {
    diagLogVersion();
    ...
    if (policy.network) {
        ...
        if (policy.network.proxy && platform === 'linux') {
            const linuxProxySupported = containment === 'bubblewrap' || containment === 'process';
            if (!linuxProxySupported) {
                throw new Error(`Proxy configuration is not supported on Linux containment='${containment}'. ` +
        ...
        const resolvesToHostFilteringBackend = containment === 'wslc' ||
 ...
 */
export function buildSandboxPayload(script, policy, workingDirectory, containerName, containment = "process") {
 ...
 */
function injectEnvIntoConfig(config, env) {
    if (!config.process) {
        config.process = { commandLine: '' };
    }
    const entries = config.process.env ? [...config.process.env] : [];
 ...
 */
function spawnWithConfig(config, options, workingDirectory, env) {
 ...
 * ```
 */
export function spawnSandbox(script, policy, options = {}, workingDirectory, containerName, env) {
...
export function spawnSandboxFromConfig(config, options = {}, workingDirectory, env) {
 ...
 * ```
 */
export function spawnSandboxAsync(script, policy, options = {}, workingDirectory, containerName) {
 ...
 */
function tryParseErrorEnvelopeFromLines(output) {
