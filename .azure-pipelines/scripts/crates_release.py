...
def _template_crate_order(template_path: str) -> list[str]:
    ...
    python3 does with no third-party module. The block it reads is generated by
    `order`, so its shape is known exactly.
    """
    with open(template_path, encoding="utf-8") as handle:
        lines = handle.read().splitlines()

    for index, line in enumerate(lines):
        if line.strip() != "- name: crateOrder":
            continue
        for cursor in range(index, len(lines)):
            if lines[cursor].strip() == "default:":
                names = []
                indent = None
                for entry in lines[cursor + 1:]:
                    stripped = entry.strip()
                    if not stripped.startswith("- "):
                        break
                    # The next *parameter* is also a "- " item, just outdented.
                    # Indentation is the only thing separating it from a crate
                    # name, so anchor on the first entry's indent and stop the
                    # moment it changes.
                    entry_indent = len(entry) - len(entry.lstrip())
                    if indent is None:
                        indent = entry_indent
                    elif entry_indent != indent:
                        break
                    names.append(stripped[2:].strip())
                return names
        break
    raise RuntimeError(f"no crateOrder default found in {template_path}")


def cmd_check_template(args: argparse.Namespace) -> int:
    """CI guard: the template's crateOrder must equal the computed order.
    ...
    actual = _template_crate_order(args.template_path)
